...
HUD 画布按 1920×480 设计，但会**自动等比缩放居中**铺进你副屏的实际分辨率：启动器用检测到的真实屏幕尺寸开窗，页面再把画布等比缩放到填满窗口（保宽高比、不裁切）。所以**不同尺寸的副屏开箱即可用**——和 1920×480 比例一致的铺满，比例不同的留对称黑边。
...
4. **全屏 + 常亮**：浏览器「添加到主屏幕」全屏铺满（iOS 已加 `apple-mobile-web-app-capable`，主屏图标启动即无工具栏）。
   **注意**：局域网走的是 http，非安全上下文，浏览器的 Wake Lock API 用不了（只在 localhost/https 生效），
   所以请在设备上把自动锁屏关掉：iOS「设置 → 显示与亮度 → 自动锁定 → 永不」，安卓「显示 → 休眠 → 永不/最长」。
